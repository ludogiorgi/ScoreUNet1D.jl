...
function construct_phi_sigma(Q::AbstractMatrix,
    ...
    # V_{j,k} = Σ_n x_j^n π_n [s_n]_k
    # Matrix form: V = centers * Diagonal(π) * score'
    V = centers * Diagonal(π_norm) * score_at_centers'

    display("Eigvals M: " * string(eigvals(M)))
    display("Eigvals V: " * string(eigvals(V)))

    @info "Constructing Φ matrix" D = D n_states = n_states rank_V = rank(V)

    # Solve M = Φ Vᵀ in a Tikhonov-regularized least-squares sense.
    # Let A = Vᵀ. For each row r of Φ we solve
    #     r A ≈ m   with ridge term λ‖r‖²,
    # which yields the closed form
    #     Φ = M Aᵀ (A Aᵀ + λ I)⁻¹.
    Vt = Matrix(V')              # A = Vᵀ
    G = Vt * Vt' + regularization * I # G = A Aᵀ + λ I
    Φ = - (M * Vt') / G            # M V / (Vᵀ V + λ I) # QUESTION: WHY I HAD TO USE MINS SIGN HERE?
    # Φ = M * pinv(V')
    Φ += regularization * I
    # Φ = - M
    ...
    # We rely solely on the Tikhonov regularization above; no eigenvalue clamping.
    chol_L = cholesky(Symmetric(Φ_S), check = true).U
    Σ = Matrix(chol_L)

    return Φ, Σ
end

function construct_phi_sigma2(Q::AbstractMatrix,
                             centers::AbstractMatrix,
                             π::AbstractVector,
                             score_at_centers::AbstractMatrix;
                             regularization::Real = 1e-6,
...
"""
    evaluate_score_at_centers(model, trainer_cfg, centers, dataset)

Evaluate the score model at each cluster center.
"""
...
# Evaluate score at cluster centers (low dataset)
@info "Evaluating score at cluster centers..."
score_at_centers = evaluate_score_at_centers(model, trainer_cfg, centers, hr_dataset)

## Construct Φ and Σ from Q
@info "Constructing Φ and Σ from Q..."
Φ, Σ = construct_phi_sigma(Q, centers, pi_vec, score_at_centers;
                           regularization = 1e-3,
                           drift_scale    = LANGEVIN_DRIFT_SCALE)
Φ2, Σ2 = construct_phi_sigma2(Q, centers, pi_vec, score_at_centers;
                           regularization = 1e-3,
                           drift_scale    = LANGEVIN_DRIFT_SCALE)                           
...
plt1 = Plots.heatmap(Φ, title = "Drift matrix Φ", colorbar_title = "Value")
plt2 = Plots.heatmap(Σ, title = "Diffusion factor Σ", colorbar_title = "Value")
plt3 = Plots.heatmap(Φ2, title = "Drift matrix Φ2", colorbar_title = "Value")
plt4 = Plots.heatmap(Σ2, title = "Diffusion factor Σ2", colorbar_title = "Value")
display(Plots.plot(plt1, plt2, plt3, plt4, layout = (2,2)))
##
π_norm = pi_vec ./ sum(pi_vec)
score_at_centers = evaluate_score_at_centers(model, trainer_cfg, centers, low_dataset)
M = centers * Q * Diagonal(π_norm) * centers'
V = centers * Diagonal(π_norm) * score_at_centers'
Plots.plot(real.(eigvals(V)))
Plots.heatmap(V, title = "Matrix V", colorbar_title = "Value")
Plots.heatmap(M, title = "Matrix M", colorbar_title = "Value")
...
@info "Integrating Langevin SDE with Φ, Σ" dt = DT_HR steps = LANGEVIN_TOTAL_STEPS burn_in = LANGEVIN_BURN_IN ensembles = n_ens device = device_str
...
@time traj_state = integrator(x0, Φ_test, Σ_test;
                              ...
                              boundary   = boundary,
                              progress   = true,
                              progress_desc = "Langevin ensemble integration") # QUESTION: HERE I'M USING AN EFFECTIVE DT OF 1 (dt*resolution). I'M HOWEVER COMPARING WITH DATA AT DT=0.1 (DT_HR). WHY I GET GOOD MATCH?
...
fig_langevin = plot_langevin_vs_observed(langevin_traj[:,:,1:1:end], low_dataset; max_lag = 50)
