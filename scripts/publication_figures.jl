...
const MODEL_PATH = joinpath(PROJECT_ROOT, "data", "model.bson")
const PHI_SIGMA_PATH = joinpath(PROJECT_ROOT, "data", "phi_sigma.hdf5")
const FIG_CACHE_PATH = joinpath(PROJECT_ROOT, "data", "publication_figures_payload.hdf5")
const LANGEVIN_SAVE_PATH = joinpath(PROJECT_ROOT, "data", "ks_lang.hdf5")
const FIG1_PATH = joinpath(PROJECT_ROOT, "figures", "publication_langevin_vs_data.png")
const FIG2_PATH = joinpath(PROJECT_ROOT, "figures", "publication_operators.png")
const DATASET_KEY = "timeseries"
# Toggle to reuse cached payload (no integration when true)
const USE_SAVED_PAYLOAD = false

# Langevin integration parameters (shared equation, two schedules)
# First run feeds PDFs/heatmaps, second run feeds ACFs; device for each is selectable.
# Devices must be either "cpu" or "gpu".
const LANGEVIN_DEVICE_PDF = "gpu"
const LANGEVIN_DEVICE_ACF = "gpu"

# Run 1 (PDFs / heatmaps)
const LANGEVIN_DT_1 = 5.0e-3
const LANGEVIN_RESOLUTION_1 = 200                  # snapshots every 0.1 units
const LANGEVIN_EFFECTIVE_DT_1 = LANGEVIN_DT_1 * LANGEVIN_RESOLUTION_1
const LANGEVIN_N_STEPS_1 = 100_000              # total integration steps
const LANGEVIN_BURN_IN_1 = 10_000
const LANGEVIN_N_ENSEMBLES_1 = 256
const LANGEVIN_GPU_ID_1 = 0                      # used when device is "gpu"

# Run 2 (ACFs)
const LANGEVIN_DT_2 = 5.0e-3
const LANGEVIN_RESOLUTION_2 = 200                  # can be tuned independently
const LANGEVIN_EFFECTIVE_DT_2 = LANGEVIN_DT_2 * LANGEVIN_RESOLUTION_2
const LANGEVIN_N_STEPS_2 = 100_000
const LANGEVIN_BURN_IN_2 = 10_000
const LANGEVIN_N_ENSEMBLES_2 = 256
const LANGEVIN_GPU_ID_2 = 1                      # used when device is "gpu"

const LANGEVIN_BOUNDARY = false  # set to `false`/`nothing` to disable, or (min, max) tuple to enforce
...
if USE_SAVED_PAYLOAD && isfile(FIG_CACHE_PATH)
    ...
    ############################
    # Langevin integrations
    #   1. PDF/heatmap run (schedule 1; selectable device/GPU id)
    #   2. ACF run (schedule 2; selectable device/GPU id)
    ############################
    clear_state_cache!("cpu"); clear_state_cache!("gpu")
    @assert LANGEVIN_DEVICE_PDF in ("cpu", "gpu") "LANGEVIN_DEVICE_PDF must be \"cpu\" or \"gpu\""
    @assert LANGEVIN_DEVICE_ACF in ("cpu", "gpu") "LANGEVIN_DEVICE_ACF must be \"cpu\" or \"gpu\""
    maybe_switch_gpu!(LANGEVIN_DEVICE_PDF, LANGEVIN_GPU_ID_1)
    integrator_pdf = build_integrator(model, sigma_model, L, C, D, LANGEVIN_DEVICE_PDF)
    maybe_switch_gpu!(LANGEVIN_DEVICE_ACF, LANGEVIN_GPU_ID_2)
    integrator_acf = build_integrator(model, sigma_model, L, C, D, LANGEVIN_DEVICE_ACF)

    x0_pdf = Matrix{Float32}(undef, D, LANGEVIN_N_ENSEMBLES_1)
    x0_acf = Matrix{Float32}(undef, D, LANGEVIN_N_ENSEMBLES_2)
    @inbounds for i in 1:LANGEVIN_N_ENSEMBLES_1
        idx = rand(rng, 1:B)
        x0_pdf[:, i] = reshape(@view(data_clean[:, :, idx]), D)
    end
    @inbounds for j in 1:LANGEVIN_N_ENSEMBLES_2
        idx = rand(rng, 1:B)
        x0_acf[:, j] = reshape(@view(data_clean[:, :, idx]), D)
    end

    @assert LANGEVIN_EFFECTIVE_DT_1 > 0 "PDF run effective dt must be positive"
    @assert LANGEVIN_EFFECTIVE_DT_2 > 0 "ACF run effective dt must be positive"

    Identity = Matrix{Float32}(I, D, D)
    boundary = normalize_boundary(LANGEVIN_BOUNDARY)

    # 1) PDF/heatmap integration
    maybe_switch_gpu!(LANGEVIN_DEVICE_PDF, LANGEVIN_GPU_ID_1)
    traj_state_pdf = integrator_pdf(x0_pdf, Float32.(Identity), Float32.(Identity);
                                    dt = LANGEVIN_DT_1,
                                    n_steps = LANGEVIN_N_STEPS_1,
                                    burn_in = LANGEVIN_BURN_IN_1,
                                    resolution = LANGEVIN_RESOLUTION_1,
                                    boundary = boundary,
                                    progress = false,
                                    progress_desc = "Langevin PDF run (dt=$(LANGEVIN_DT_1), device=$(LANGEVIN_DEVICE_PDF))")
    traj_pdf = Array(traj_state_pdf)

    # 2) ACF integration
    maybe_switch_gpu!(LANGEVIN_DEVICE_ACF, LANGEVIN_GPU_ID_2)
    traj_state_acf = integrator_acf(x0_acf, Float32.(Phi_scaled), Float32.(Sigma_scaled);
                                    dt = LANGEVIN_DT_2,
                                    n_steps = LANGEVIN_N_STEPS_2,
                                    burn_in = LANGEVIN_BURN_IN_2,
                                    resolution = LANGEVIN_RESOLUTION_2,
                                    boundary = boundary,
                                    progress = false,
                                    progress_desc = "Langevin ACF run (dt=$(LANGEVIN_DT_2), device=$(LANGEVIN_DEVICE_ACF))")
    traj_acf = Array(traj_state_acf)

    T_snap_pdf = size(traj_pdf, 2)
    traj_tensor_pdf = reshape(traj_pdf, L, C, T_snap_pdf, :)
    langevin_tensor_pdf = reshape(traj_tensor_pdf, L, C, :)

    T_snap_acf = size(traj_acf, 2)
    traj_tensor_acf = reshape(traj_acf, L, C, T_snap_acf, :)
    langevin_tensor_acf = traj_tensor_acf

    ############################
    # Persist Langevin trajectories
    ############################
    h5open(LANGEVIN_SAVE_PATH, "w") do h5
        # Trajectories and metadata (kept under legacy key "trajectory" for the PDF run)
        write(h5, "trajectory", traj_tensor_pdf)  # (L, C, snapshots, ensembles)
        write(h5, "trajectory_pdf", traj_tensor_pdf)
        write(h5, "trajectory_acf", traj_tensor_acf)
        write(h5, "device_pdf", LANGEVIN_DEVICE_PDF)
        write(h5, "device_acf", LANGEVIN_DEVICE_ACF)
        write(h5, "gpu_id_pdf", Int(LANGEVIN_GPU_ID_1))
        write(h5, "gpu_id_acf", Int(LANGEVIN_GPU_ID_2))
        write(h5, "dt_pdf", Float64(LANGEVIN_DT_1))
        write(h5, "resolution_pdf", Int(LANGEVIN_RESOLUTION_1))
        write(h5, "dt_effective_pdf", Float64(LANGEVIN_EFFECTIVE_DT_1))
        write(h5, "n_steps_pdf", Int(LANGEVIN_N_STEPS_1))
        write(h5, "burn_in_pdf", Int(LANGEVIN_BURN_IN_1))
        write(h5, "n_ensembles_pdf", Int(LANGEVIN_N_ENSEMBLES_1))
        write(h5, "dt_acf", Float64(LANGEVIN_DT_2))
        write(h5, "resolution_acf", Int(LANGEVIN_RESOLUTION_2))
        write(h5, "dt_effective_acf", Float64(LANGEVIN_EFFECTIVE_DT_2))
        write(h5, "n_steps_acf", Int(LANGEVIN_N_STEPS_2))
        write(h5, "burn_in_acf", Int(LANGEVIN_BURN_IN_2))
        write(h5, "n_ensembles_acf", Int(LANGEVIN_N_ENSEMBLES_2))
    end

    ############################
    # Data products for figures
    ############################
    n_hm = min(HEATMAP_SNAPSHOTS, size(traj_tensor_pdf, 3))
    heatmap_langevin = Array(traj_tensor_pdf[:, 1, 1:n_hm, 1])
    ...
    downsample_factor = max(1, round(Int, 1.0 / LANGEVIN_EFFECTIVE_DT_2))
    ...
    dt_langevin = LANGEVIN_EFFECTIVE_DT_2 * downsample_factor * ACF_STRIDE
...
# Align ACF ranges to 0..100 time units
acf_tmax = 100.0
